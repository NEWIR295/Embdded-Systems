...
/* Array of DDR Registers */
volatile uint8* const DDR_REG[NUM_OF_PORTS] = { &DDRA, &DDRB, &DDRC, &DDRD };
...
void GPIO_setupPortDirectionMasked(uint8 portNum, uint8 direction, uint8 mask){
	if(portNum >= NUM_OF_PORTS){
		...
		uint8 currentDDR = *DDR_REG[portNum]; // Assume an array of DDR addresses
		uint8 newDDR = (currentDDR & ~mask) | (direction & mask);
